    ...
    try:
        with ConnectHandler(**get_params(port)) as tn:
            ...
            cmds = [
                ...
                "sysctl -p",
                "ip addr add 11.0.0.1/24 dev eth0 || true",
                "ip addr add 10.0.0.1/24 dev eth1 || true",
                "ip addr add 10.1.0.1/24 dev eth2 || true",
                "ip link set eth0 up",
                "ip link set eth1 up",
                "ip link set eth2 up"
            ]
            ...
            print("OK: AlpineRouter paruoštas.")
        return True
...
def configure_ovs_logic(node_name, port):
    ...
    try:
        with ConnectHandler(**get_params(port)) as tn:
            ...
            if node_name == "Main1": setup_cmds.append("ip addr add 10.0.0.100/24 dev br-final")
            elif node_name == "Support": setup_cmds.append("ip addr add 10.1.0.100/24 dev br-final")
            elif node_name == "Admin": setup_cmds.append("ip addr add 11.0.0.100/24 dev br-final")

            for cmd in setup_cmds:
                tn.send_command(cmd, expect_string=r'[#$]')
            print(f"OK: {node_name} paruoštas.")
        return True
    except Exception as e:
...
def configure_alpine_logic(name, port, ip, gw):
    """Konfiguruoja galinius Alpine mazgus."""
    print(f"\n[ALPINE] {name} -> IP: {ip}")
    try:
        with ConnectHandler(**get_params(port)) as tn:
            tn.write_channel("\n")
